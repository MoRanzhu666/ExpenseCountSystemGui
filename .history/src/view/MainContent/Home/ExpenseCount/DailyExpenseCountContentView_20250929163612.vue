<template>
  <div>
    <el-table :data="tableData" border :row-key="(row) => row.id" height="600vh">
      <el-table-column
...
const getTableData = async () => {
  ...
  tableData.value.forEach(async (item)=>{
    item.createByName = (await getUserInfo(item.createBy)).name
    item.updateByName = (await getUserInfo(item.updateBy)).name
  })
