    ...
    <div style="overflow: auto; width: 100%; height: 100%">
      <el-form style="padding: 5px 50px" label-width="auto">
        ...
        <el-form-item
          ...
          :label="item.lable"
        >
          <el-select v-if="item.type=='selection'" v-model="item.value" placeholder="请选择">
            <el-option
              v-for="option in item.categoryOptions.value"
              :key="option.value"
              :label="option.label"
              :value="option.value"
            />
          </el-select>
          <el-input v-if="item.type!='selection'" v-model="item.value" :type="item.type" :precision="item.precision" @keydown.enter="handleAdd(processFormData(formData))"></el-input>
        </el-form-item>
      </el-form>
    </div>
    <div class="close">
